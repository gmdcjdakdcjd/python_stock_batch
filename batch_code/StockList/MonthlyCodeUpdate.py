            ...
            if rs[0] is None or rs[0].strftime('%Y-%m-%d') < today:
                ...
                for idx in range(len(etf)):
                    ...
                    sql = (
                        ...
                        "(std_code, code, name, base_index, index_provider, leverage, replication_method, market_type, asset_type, manager, tax_type, last_update) "
                        f"VALUES ('{std_code}', '{code}', '{name}', '{base_index}', '{index_provider}', '{leverage}', '{replication_method}', '{market_type}', '{asset_type}', '{manager}', '{tax_type}', '{today}')"
                    )
                    curs.execute(sql)
                    self.codes[code] = name
                    tmnow = datetime.now().strftime('%Y-%m-%d %H:%M')
                    print(
                        f"[{tmnow}] #{idx + 1:04d} REPLACE INTO etf_info VALUES ({std_code}, {code}, {name}, {base_index}, {index_provider}, {leverage}, {replication_method}, {market_type}, {asset_type}, {manager}, {tax_type}, {today})")
                self.conn.commit()
                print('')
            ...
            if rs[0] is None or rs[0].strftime('%Y-%m-%d') < today:
                ...
                for idx in range(len(krx)):
                    ...
                    sql = (
                        ...
                        f"VALUES ('{code}', '{name}', '{market_type}', '{security_type}', '{stock_type}', '{std_code}', '{today}')"
                    )
                    curs.execute(sql)
                    self.codes[code] = name
                    tmnow = datetime.now().strftime('%Y-%m-%d %H:%M')
                    print(
                        f"[{tmnow}] #{idx + 1:04d} REPLACE INTO company_info VALUES ({code}, {name}, {market_type}, {security_type}, {stock_type}, {std_code}, {today})")
                self.conn.commit()
                print('')
