...
class GoldKRWDBUpdater:
    ...
    # ------------------------------------------------------------------------------------
    # 1) 국내 금(goldDailyQuote) 단일 페이지 스크래핑
    # ------------------------------------------------------------------------------------
    def read_gold_krw_page(self, page=1):
        try:
            ...
            print("Exception (GOLD_KRW PAGE):", e)
            return pd.DataFrame()
    ...
    def replace_gold_into_db(self, df):
        ...
        with self.conn.cursor() as curs:
            for r in df_sorted.itertuples():
                ...
                VALUES ('GOLD_KR', '{r.date}', {r.close}, {r.change_amount}, {r.change_rate});
                """
                curs.execute(sql)

        self.conn.commit()
        print(f"\n[INFO] GOLD_KR {len(df)} rows 저장 완료")
