...
class DualMomentumBatch:
    ...
    def run_dual_momentum_batch(self, start_date, end_date):
        ...
        def adjust_date(date_str):
            sql = f"SELECT MAX(date) FROM daily_price_us WHERE date <= '{date_str}'"
            cursor.execute(sql)
        ...
        for code, name in self.mk.codes.items():
            try:
                cursor.execute(f"SELECT close FROM daily_price_us WHERE code='{code}' AND date='{start_date}'")
                start_val = cursor.fetchone()
                cursor.execute(f"SELECT close FROM daily_price_us WHERE code='{code}' AND date='{end_date}'")
        ...
        # ✅ 콘솔 리포트 출력
        print(f"📈 [DUAL MOMENTUM - 1M] 상대모멘텀 상위 {self.TOP_RELATIVE}개 → 절대모멘텀({self.MIN_ABS_RETURN:.1f}%) 통과 후 상위 {self.FINAL_TOP}개:\n")
        if df_final.empty:
...
if __name__ == '__main__':
    ...
    start = (today - timedelta(days=30)).strftime('%Y-%m-%d')  # ✅ 1개월(30일)로 단축
