...
for s in stocks:
    try:
        ...
        df.dropna(subset=['upper', 'close'], inplace=True)
        if df.empty:
            continue
        ...
        # - 종가 ≥ $10
        # - 상단선보다 살짝 아래 or 근처
        if -1.0 <= diff_rate <= 1.0 and close_price >= 10 and close_price <= upper_band * 1.01:
...
if touch_candidates:
    ...
    print("📈 [일봉] 볼린저밴드 상단 터치 종목 리스트 (±1%, 종가 ≥ $10):\n")
    ...
    signal_type = "SELL"  # 상단 터치는 매도 관점으로 분류
